...
| **Access reviews** | Create certification **campaigns**, open/close them, and **certify or revoke** each access item. A campaign can also certify **delegations** — the standing permission for an agent to act on behalf of a user — which the item row names (`via <agent> · <purpose>`) because certifying a scope list without knowing who holds it is not a review. They are **off by default**: nobody leaves the company on an agent's behalf, so a forgotten delegation is invisible in the ordinary course of things, but adding one to a campaign should be a choice and not a surprise. Signals flag the ones to look at first — dormant, **agent suspended** (lift the suspension and every delegation comes back with nobody re-confirming), expiring, consent given at AAL1. Needs [`laravel-iam-agents`](https://github.com/padosoft/laravel-iam-agents) ≥ 1.6 on `laravel-iam-server` ≥ 1.27; without them the checkbox simply isn't there. |
...
| **Decision playground** | Ask the PDP a **`check` / `explain`** and see the real ALLOW/DENY + the reasoning. Fill the **act chain** (`agent:ID`, comma-separated, current actor first) and the same question becomes the *delegated* one — "may this agent do it **for** that user" — answered with **one sub-decision id per hop** (iam-agents ≥ 1.5). The strict intersection means a single denying hop denies everything, so the panel names *which* one instead of only listing the chain. |
